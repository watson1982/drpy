...
public class Thunder {
    ...
    private static void init(Context context) {
        ...
        cacheRoot = context.getCacheDir()
            .getAbsolutePath() + File.separator + "thunder";
    }
                    ...
                    if (urlInfo != null) {
                        ...
                        for (Movie.Video.UrlBean.UrlInfo.InfoBean infoBean: urlInfo.beanList) {
                            ...
                            if (isMagnet(url) || isThunder(url) || isTorrent(url)) {
                                if (isThunder(url)) url = XLDownloadManager.getInstance()
                                    .parserThunderUrl(url);
                                String link = isThunder(url) ? XLDownloadManager.getInstance()
                                    .parserThunderUrl(url) : url;
                                Uri p = Uri.parse(link);
                                if (p == null) {
                                    continue;
                                }
                                String fileName = XLTaskHelper.instance()
                                    .getFileName(link);
                                ...
                                try {
                                    ...
                                    currentTask = isMagnet(url) ? XLTaskHelper.instance()
                                        .addMagentTask(url, cacheRoot, fileName) : XLTaskHelper.instance()
                                        .addThunderTask(url, cacheRoot, fileName);
                                ...
                                outerLoop: while (true) {
                                    ...
                                    XLTaskInfo taskInfo = XLTaskHelper.instance()
                                        .getTaskInfo(currentTask);
                                    if (taskInfo != null) {
                                        switch (taskInfo.mTaskStatus) {
                                            case 2:
                                                {
                                                    try {
                                                        TorrentInfo torrentInfo = XLTaskHelper.instance()
                                                            .getTorrentInfo(cache.getAbsolutePath());
                                ...
                                if (isNetworkDownloadTask(url)) {
                                    ...
                                    name = XLTaskHelper.instance()
                                        .getFileName(task_url);
    ...
    public static boolean play(String url, ThunderCallback callback) {
        if (url.startsWith("tvbox-torrent:")) {
            ...
            threadPool.execute(new Runnable() {@Override
                public void run() {
                    String torrentName = new File(info.torrentPath)
                        .getName();
                    String cache = cacheRoot + File.separator + torrentName.substring(0, torrentName.lastIndexOf("."));
                    currentTask = XLTaskHelper.instance()
                        .addTorrentTask(info.torrentPath, cache, info.mFileIndex);
                    ...
                    while (true) {
                        ...
                        XLTaskInfo taskInfo = XLTaskHelper.instance()
                            .getBtSubTaskInfo(currentTask, info.mFileIndex)
                            .mTaskInfo;
                        switch (taskInfo.mTaskStatus) {
                            case 3:
                                {
                            ...
                            case 2:
                                { // 下载完成
                                    String pUrl = XLTaskHelper.instance()
                                        .getLoclUrl(cache + File.separator + info.mFileName);
                                    callback.play(pUrl);
                                    return;
                                }
                        }
        ...
        if (url.startsWith("tvbox-oth:")) {
            ...
            currentTask = XLTaskHelper.instance()
                .addThunderTask(task_url, localPath, null);
    ...
    static String randomMac() {
        return randomString("ABCDEF0123456", 12)
            .toUpperCase();
    }
    ...
    public static void stopTask() {
        if (currentTask != 0L) {
            XLTaskHelper.instance()
                .deleteTask(currentTask, task_url.isEmpty() ? cacheRoot : localPath);
            currentTask = 0L;
        }
    }

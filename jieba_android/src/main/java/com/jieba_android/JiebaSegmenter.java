...
import com.jieba_android.viterbi.FinalSeg;
...
public class JiebaSegmenter {
    ...
    public ArrayList<String> getDividedString(String query) {
        ...
        for (int i = 0; i < lst.size(); i++) {
            String segment = lst.get(i).word;
            //停用词不予考虑，单字词不予考虑
            if(!wordDict.stopWordsSet.contains(segment) && segment.length()>1) {
                freqMap.put(segment, 1.0);
            }
        }
